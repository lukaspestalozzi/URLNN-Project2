...
class MountainCar():
    ...
    def __init__(self, g=10.0, d=100.0, H=10., m=10.0,
        ...
        # load caches
        self._cache_steps = 10000
        print("loading caches...")
        t = time()
        self._h_cache = self.load_cache('h_cache.json')
        self._h_prime_cache = self.load_cache('h_prime_cache.json')
        self._h_second_cache = self.load_cache('h_second_cache.json')
        print("...done", "t:", time() - t)

    def load_cache(self, filename):
        with open('caches/'+filename) as cf:
            return np.array(json.load(cf))

    def _create_caches(self, x_min=-150, x_max=30):
        print("creating caches...")
        h_cache = [self._h(x+ix/self._cache_steps) for x in range(x_min, x_max) for ix in range(0, self._cache_steps)]
        print("...h done...")
        h_prime_cache = [self._h_prime(x+ix/self._cache_steps) for x in range(x_min, x_max) for ix in range(0, self._cache_steps)]
        print("...hprime done...")
        h_second_cache = [self._h_second(x+ix/self._cache_steps) for x in range(x_min, x_max) for ix in range(0, self._cache_steps)]
        print("...hsecond done...")
        print("writing...")
        with open('caches/h_cache.json', 'w') as hf:
            json.dump(h_cache, hf)
        print("...h done...")
        with open('caches/h_prime_cache.json', 'w') as pf:
            json.dump(h_prime_cache, pf)
        print("...hprime done...")
        with open('caches/h_second_cache.json', 'w') as sf:
            json.dump(h_second_cache, sf)
        print("...hsecond done...")
        print("...done")

    def _cache_pos(self, x):
        return int((x+150)*self._cache_steps)
    ...
    def _h(self, x):
        ...
        """
        return self._h_cache[self._cache_pos(x)]
        """
        res = (x - self.d)**2 * (x + self.d)**2 / ((self.d**4/self.H)+x**2)
        return res
        """

    def _h_array(self, x):
        """Return the value of the landscape function h in x for all elements in X.
        """
        res = (x - self.d)**2 * (x + self.d)**2 / ((self.d**4/self.H)+x**2)
        return res
    ...
    def _h_prime(self, x):
        ...
        """
        return self._h_prime_cache[self._cache_pos(x)]
        """
        c = self.d**4/self.H
        res = 2 * x * (x**2 - self.d**2) * (2*c + self.d**2 + x**2) / (c+x**2)**2
        return res
        """

    def _h_prime_array(self, x):
        """Return the value of the first derivative of the landscape function h in x for all elements in X.
        """
        c = self.d**4/self.H
        res = 2 * x * (x**2 - self.d**2) * (2*c + self.d**2 + x**2) / (c+x**2)**2
        return res
    ...
    def _h_second(self, x):
        ...
        """
        return self._h_second_cache[self._cache_pos(x)]
        """
        c = self.d**4/self.H
        res = 2 * (
    ...
    def _h_second_array(self, x):
        """Return the value of the second derivative of the landscape function h in x for all elements in X.
        """
        c = self.d**4/self.H
        res = 2 * (
            - 2 * c**2 * (self.d**2 - 3*x**2)
    ...
    def _energy_array(self, x, x_d):
        """Return the total energy of the car with variable x and x_d.
        """
        # note that v and x dot are not the same: v includes the y direction!
        return self.m * (self.g * self._h_array(x) + 0.5 * (1 + self._h_prime_array(x)**2) * x_d**2)
...
class MountainCarViewer():
    ...
    def _plot_energy_landscape(self, ax=None):
        ...
        E = self.mountain_car._energy_array(X, XD)
